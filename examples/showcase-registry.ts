export type ExampleGroup = 'Geometry' | 'Materials' | 'Textures' | 'Cameras' | 'Interaction' | 'Simulation' | 'Extensions';
export type ExampleId = 'primitives' | 'materials' | 'scene-graph' | 'procedural-textures' | 'wood-texture' | 'brick-texture' | 'cloth-texture' | 'cameras' | 'raycasting' | 'physics' | 'texture' | 'postprocess';
...
export const examples: ExampleMeta[] = [
  ...
  { id: 'raycasting', group: 'Interaction', title: 'Raycast playground', summary: 'Click a textured volume to cast a ray and select the nearest hit.', hint: 'Click objects, drag to orbit' },
...
export function nextExample(current: ExampleId): ExampleId { const index = examples.findIndex((example) => example.id === current); return examples[(index + 1 + examples.length) % examples.length].id; }
