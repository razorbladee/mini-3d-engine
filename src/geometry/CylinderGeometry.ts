...
export class CylinderGeometry extends BufferGeometry {
  constructor(radius = 1, height = 2, segments = 24) {
    ...
    const u: number[] = [];
    const half = height / 2;
    const push = (v: number[], normal: number[], uv: number[]) => {
      p.push(...v);
      n.push(...normal);
      u.push(...uv);
    };
    for (let i = 0; i < segments; i++) {
      const a0 = (i / segments) * Math.PI * 2,
        ...
        s1 = Math.sin(a1);
      const side = [
        [radius * c0, -half, radius * s0],
        [radius * c1, half, radius * s1],
        [radius * c1, -half, radius * s1],
        [radius * c0, -half, radius * s0],
        [radius * c0, half, radius * s0],
        [radius * c1, half, radius * s1],
      ...
      for (const [x, y, z] of side)
        push([x, y, z], [x / radius, 0, z / radius], [(Math.atan2(z, x) / (Math.PI * 2) + 1) % 1, (y + half) / height]);
      for (const [x, y, z] of [
        [0, -half, 0],
        [radius * c0, -half, radius * s0],
        [radius * c1, -half, radius * s1],
      ])
        push([x, y, z], [0, -1, 0], [x / radius / 2 + 0.5, z / radius / 2 + 0.5]);
      for (const [x, y, z] of [
        [0, half, 0],
        [radius * c1, half, radius * s1],
        [radius * c0, half, radius * s0],
