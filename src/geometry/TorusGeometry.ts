  ...
  constructor(radius = 1, tube = 0.35, radialSegments = 32, tubularSegments = 16) {
    ...
    const point = (a: number, b: number) => {
      ...
      return {
        ...
        n: [cb * ca, sb, cb * sa],
        uv: [a / (Math.PI * 2), b / (Math.PI * 2)],
      };
    };
    for (let i = 0; i < radialSegments; i++)
      for (let j = 0; j < tubularSegments; j++) {
        const a0 = (i / radialSegments) * Math.PI * 2,
          a1 = ((i + 1) / radialSegments) * Math.PI * 2,
          b0 = (j / tubularSegments) * Math.PI * 2,
          b1 = ((j + 1) / tubularSegments) * Math.PI * 2;
        for (const v of [point(a0, b0), point(a1, b1), point(a1, b0), point(a0, b0), point(a0, b1), point(a1, b1)]) {
          p.push(...v.p);
          n.push(...v.n);
          u.push(...v.uv);
        }
