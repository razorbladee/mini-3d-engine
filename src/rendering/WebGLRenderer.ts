...
import { AmbientLight, DirectionalLight, PointLight, SpotLight, HemisphereLight } from '../lights/Light';
...
type ProgramState = {
  ...
  hasMapLocation: WebGLUniformLocation | null;
...
type LightState = {
  ambient: number[];
  directional: { color: number[]; direction: number[]; intensity: number }[];
  point: { color: number[]; position: number[]; intensity: number; distance: number }[];
};
function normalMatrix(elements: Float32Array) {
  const a = elements[0],
    b = elements[1],
    c = elements[2],
    d = elements[4],
...
export class WebGLRenderer {
  ...
  private uploaded = new WeakMap<object, WebGLBuffer>();
  private uploadedNormals = new WeakMap<object, WebGLBuffer>();
  private uploadedUvs = new WeakMap<object, WebGLBuffer>();
  ...
  private createProgram(vs: string, fs: string, lit: boolean) {
    ...
    const state: any = {
      ...
      mapLocation: gl.getUniformLocation(program, 'uMap'),
      hasMapLocation: gl.getUniformLocation(program, 'uHasMap'),
    };
    if (lit) {
      for (const key of [
        'roughness',
        ...
        state[`${key}Location`] = gl.getUniformLocation(
          program,
          `u${key[0].toUpperCase() + key.slice(1)}${key.includes('Color') || key.includes('Direction') || key.includes('Intensity') ? '[0]' : ''}`,
        );
    }
    return state;
  }
  private color(v: string) {
    const h = v.replace('#', '');
    const n = parseInt(
      h.length === 3
        ? h
  ...
  private collectLights(scene: Scene): LightState {
    const ambient = [0, 0, 0],
      directional: any[] = [],
      point: any[] = [];
    scene.traverse((node: Node) => {
      if (node instanceof AmbientLight) {
        const c = this.color(node.color);
        ambient[0] += c[0] * node.intensity;
        ambient[1] += c[1] * node.intensity;
        ambient[2] += c[2] * node.intensity;
      } else if (node instanceof HemisphereLight) {
        const sky = this.color(node.color),
          ground = this.color(node.groundColor);
        ambient[0] += (sky[0] + ground[0]) * 0.5 * node.intensity;
        ambient[1] += (sky[1] + ground[1]) * 0.5 * node.intensity;
        ambient[2] += (sky[2] + ground[2]) * 0.5 * node.intensity;
      } else if (node instanceof SpotLight && directional.length < 4)
        directional.push({
          color: this.color(node.color),
  ...
  render(scene: Scene, camera: Camera) {
    ...
    const meshes: Mesh[] = [];
    scene.traverse((n) => {
      if (n instanceof Mesh && n.visible) meshes.push(n);
    });
    const cameraWorld = camera.worldMatrix.elements;
    meshes.sort((a, b) => {
      const az = a.worldMatrix.elements[14] - cameraWorld[14],
        bz = b.worldMatrix.elements[14] - cameraWorld[14];
      const ad = a.material.transparent ? 1 : 0,
        bd = b.material.transparent ? 1 : 0;
      return ad - bd || (ad ? bz - az : 0);
    });
    for (const node of meshes) {
      ...
      gl.uniformMatrix3fv(state.normalMatrixLocation, false, normalMatrix(node.worldMatrix.elements));
      gl.uniform4fv(state.colorLocation, node.material.color);
      const texture = node.material.map?.upload(gl);
      gl.uniform1i(state.hasMapLocation!, texture ? 1 : 0);
      if (texture) {
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.uniform1i(state.mapLocation!, 0);
      }
      if (state === this.lit) {
        const e = camera.worldMatrix.elements;
        gl.uniform3fv(state.cameraPositionLocation!, e.slice(12, 15));
        ...
        gl.uniform3fv(
          state.directionalColorLocation!,
          lights.directional.flatMap((l) => l.color),
        );
        gl.uniform3fv(
          state.directionalDirectionLocation!,
          lights.directional.flatMap((l) => l.direction),
        );
        gl.uniform1fv(
          state.directionalIntensityLocation!,
          lights.directional.map((l) => l.intensity),
        );
        gl.uniform1i(state.pointCountLocation!, lights.point.length);
        gl.uniform3fv(
          state.pointColorLocation!,
          lights.point.flatMap((l) => l.color),
        );
        gl.uniform3fv(
          state.pointPositionLocation!,
          lights.point.flatMap((l) => l.position),
        );
        gl.uniform1fv(
          state.pointIntensityLocation!,
          lights.point.map((l) => l.intensity),
        );
        gl.uniform1fv(
          state.pointDistanceLocation!,
          lights.point.map((l) => l.distance),
        );
        const m = node.material as StandardMaterial;
        gl.uniform1f(state.roughnessLocation!, m.roughness);
        gl.uniform1f(state.metalnessLocation!, m.metalness);
      }
      const g: any = node.geometry;
      const position = this.uploaded.get(g) || gl.createBuffer()!,
        normal = this.uploadedNormals.get(g) || gl.createBuffer()!,
        uv = this.uploadedUvs.get(g) || gl.createBuffer()!;
      this.uploaded.set(g, position);
      this.uploadedNormals.set(g, normal);
      this.uploadedUvs.set(g, uv);
      if (!g.gpuBuffer) {
        gl.bindBuffer(gl.ARRAY_BUFFER, position);
        gl.bufferData(gl.ARRAY_BUFFER, g.positions, gl.STATIC_DRAW);
        g.gpuBuffer = position;
      }
      if (!g.normalBuffer) {
        gl.bindBuffer(gl.ARRAY_BUFFER, normal);
        gl.bufferData(gl.ARRAY_BUFFER, g.normals, gl.STATIC_DRAW);
        g.normalBuffer = normal;
      }
      if (!g.uvBuffer) {
      ...
      gl.drawArrays(gl.TRIANGLES, 0, g.positions.length / 3);
    }
    gl.depthMask(true);
    gl.disable(gl.BLEND);
  }
