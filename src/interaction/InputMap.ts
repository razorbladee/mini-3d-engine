...
export class InputMap {
  ...
  constructor(private readonly element: Window | HTMLElement = globalThis.window) {
  ...
  bind(action: InputAction, keys: string[]) { return { isDown: () => keys.some((key) => this.keys.has(key)), wasPressed: () => keys.some((key) => this.pressed.has(key)), action }; }
