...
export class InputMap {
  ...
  private readonly onKeyUp = (event: Event) => {
    this.keys.delete((event as KeyboardEvent).key);
  };
  bind(action: InputAction, keys: string[]) {
    return {
      action,
      isDown: () => keys.some((key) => this.keys.has(key)),
      wasPressed: () => keys.some((key) => this.pressed.has(key)),
    };
  }
  endFrame() {
    this.pressed.clear();
