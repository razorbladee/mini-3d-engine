...
export class Node {
  ...
  updateWorldMatrix(parentMatrix?: Matrix4) {
    this.localMatrix.compose(this.position, this.scale, this.rotation);
    this.worldMatrix = parentMatrix ? parentMatrix.clone().multiply(this.localMatrix) : this.localMatrix.clone();
